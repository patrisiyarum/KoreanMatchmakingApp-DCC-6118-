...
import { conversationPromptCategories } from '../data/mockData';
import { ConversationPrompt } from '../types';
...
export function ConversationPrompts({
  ...
  const [activeCategoryId, setActiveCategoryId] = useState(
    conversationPromptCategories[0].id
  );

  const activeCategory =
    conversationPromptCategories.find(c => c.id === activeCategoryId) ||
    conversationPromptCategories[0];
  ...
  const renderPromptCard = (prompt: ConversationPrompt) => (
    <motion.div
      ...
      className="bg-white border border-neutral-200 rounded-xl p-3 hover:border-blue-400 hover:shadow-sm transition-all"
    >
      ...
      <div className="flex gap-2">
        <button
          ...
          aria-label={`Send prompt: ${prompt.english}`}
        ...
        <button
          ...
          aria-label={`Edit prompt before sending: ${prompt.english}`}
  ...
  const renderCategoryTabs = () => (
    <div className="flex gap-2 overflow-x-auto pb-2 -mx-1 px-1">
      {conversationPromptCategories.map(cat => {
