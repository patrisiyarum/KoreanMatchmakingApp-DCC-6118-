...
export function generateAvatar(user: Pick<User, 'id' | 'nativeLanguage'>): GeneratedAvatar {
  const seed = hash32(user.id);
  const palette = PASTEL_PALETTE[seed % PASTEL_PALETTE.length];
  const useHangul = user.nativeLanguage === 'Korean';
  ...
  return {
    bgColor: palette.bg,
    fgColor: palette.fg,
    eyeChar: pickFromList(seed, 1, eyeSet),
    mouthChar: pickFromList(seed, 2, mouthSet),
    eyeGapEm: pickFromRange(triangular01(seed, 10), 0.05, 0.4),
    mouthOffsetPx: pickFromRange(triangular01(seed, 20), -2, 4),
    tiltDeg: pickFromRange(triangular01(seed, 30), -6, 6),
  };
}
